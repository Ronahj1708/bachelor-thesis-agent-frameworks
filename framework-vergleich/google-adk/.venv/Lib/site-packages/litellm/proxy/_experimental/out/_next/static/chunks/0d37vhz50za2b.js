...
client = openai.OpenAI(
	api_key="${y||"YOUR_LITELLM_API_KEY"}",
	base_url="${_}"
)`;switch(f){case i.CHAT:{let e=Object.keys(C).length>0,r="";if(e){let e=JSON.stringify({metadata:C},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();r=`,
    extra_body=${e}`}let o=S.length>0?S:[{role:"user",content:w}];t=`
import base64
...
`;break}case i.RESPONSES:{let e=Object.keys(C).length>0,r="";if(e){let e=JSON.stringify({metadata:C},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();r=`,
    extra_body=${e}`}let o=S.length>0?S:[{role:"user",content:w}];t=`
import base64
